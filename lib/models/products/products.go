...
type Product struct {
	...
	Description string `gorm:"not null;type:varchar(50);unique" json:"description,omitempty"`
	...
	UserID uint `gorm:"not null;index" json:"user_id,omitempty"` // Foreign key to the Category table
...
type ProductEncrypted struct {
	...
	CategoryID string `json:"category_id,omitempty"` // Foreign key to the User table
